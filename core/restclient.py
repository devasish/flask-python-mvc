...
def post(url, data, headers, authdata = None):
    """
    Post request to api
    ...
    if headers.get('Content-Type', "") == 'application/json':
        resp = requests.post(url, json=data, headers=headers, auth=prepauth(authdata))
    else:
        headers['Content-Type'] = 'application/x-www-form-urlencoded'
        resp = requests.post(url, data=data, headers=headers, auth=prepauth(authdata))
...
def get(url, data, headers={}, authdata = None):
    """
    Get request to api
    @param url: (string) api url
    @param data : (dict) data to send
    ...
    """
    print(authdata)
    resp = requests.get(url, headers=headers, params=data, verify=False, auth=prepauth(authdata))
